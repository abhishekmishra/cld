...
cld_cmd_err net_ls_cmd_handler(void *handler_args, struct array_list *options,
	...
	if (success) {
		...
		int col_num = 0;
		int len_networks = array_list_length(networks);
		cld_table* net_tbl;
		if (create_cld_table(&net_tbl, len_networks, 4) == 0) {
			...
			for (int i = 0; i < len_networks; i++) {
				docker_network* net = (docker_network*) array_list_get_idx(
						networks, i);
...
//		}
		if (make_command(&netls_command, "list", "ls", "Docker Networks List",
			...
			array_list_add(image_command->sub_commands, netls_command);
		}
	}
