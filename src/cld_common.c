						 ...
						 zclk_command_output_handler error_handler)
{
	...
	// sprintf(res_str,"\nURL: %s\n", res->url);
	success_handler(ZCLK_COMMAND_IS_RUNNING, ZCLK_RESULT_STRING, res_str);
	if (!is_ok(res))
	{
		printf("DOCKER RESULT: Response error_code = %d, http_response = %ld\n",
			   res->error_code, res->http_error_code);
		sprintf(res_str, "ERROR: %s\n", res->message);
		error_handler(ZCLK_COMMAND_ERR_UNKNOWN, ZCLK_RESULT_STRING, res_str);
	}
	else
	{
		if (res->message)
		{
			success_handler(ZCLK_COMMAND_IS_RUNNING, ZCLK_RESULT_STRING, res->message);
		}
	}
}
